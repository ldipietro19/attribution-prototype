...
export default function TaxonomyPage() {
  ...
  return (
    <div style={{ maxWidth: "900px", margin: "0 auto", padding: "40px 32px" }}>
      <h1 style={{ fontSize: "22px", fontWeight: 700, color: "var(--text-primary)", marginBottom: "4px" }}>Taxonomy</h1>
      <p style={{ fontSize: "13px", color: "var(--text-secondary)", marginBottom: "28px" }}>Category pack configuration — drives AI attribution and DPP compliance checks.</p>
      ...
      <div style={{ background: "var(--badge-green-bg)", border: "1px solid var(--badge-green-border)", borderRadius: "10px", padding: "12px 16px", marginBottom: "24px" }}>
        ...
        <p style={{ fontSize: "12px", color: "var(--badge-green-text)", opacity: 0.8 }}>
          Configured at implementation. Changes require re-running attribution on affected styles.
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: "16px", marginBottom: "20px", flexWrap: "wrap" }}>
        {([
          { source: "ai" as FieldSource, desc: "AI attributes from image / description" },
          { source: "vendor" as FieldSource, desc: "Must be submitted by vendor — not editable by merchant" },
          { source: "plm" as FieldSource, desc: "Read-only from PLM" },
        ]).map(({ source, desc }) => (
          <div key={source} style={{ display: "flex", alignItems: "center", gap: "7px", fontSize: "12px", color: "var(--text-secondary)" }}>
      ...
      <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px 16px" }}>
        ...
        <p style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
          Field changes are made during implementation by your Attribution Platform team. Each field requires an AI prompt update and re-attribution of existing styles.
        </p>
      </div>
    </div>
  );
